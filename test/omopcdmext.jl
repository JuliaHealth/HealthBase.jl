@testset "HealthBaseOMOPCDMExt" begin
    ...
    @testset "Constructor and Type Validation" begin
        ...
        @testset "Invalid DataFrame Type Check" begin
            @test_throws ArgumentError HealthBase.HealthTable(
                person_df_bad;
                omop_cdm_version = "v5.4.1",
            )
        end
    ...
    @testset "Preprocessing Functions" begin
        ...
        @testset "one_hot_encode function" begin
            ...
            result = HealthBase.one_hot_encode(
                ht;
                cols = [:gender_concept_id],
            ...
            result_ht = HealthBase.one_hot_encode(
                ht;
                cols = [:gender_concept_id],
            ...
            @test_logs (:warn, r"Column bool_column is already Boolean") HealthBase.one_hot_encode(
                ht;
                cols = [:bool_column],
                return_features_only = true,
            )
            ...
            @test_throws AssertionError HealthBase.one_hot_encode(
                ht;
        ...
        @testset "apply_vocabulary_compression function" begin
            # Test basic functionality
            compressed = HealthBase.apply_vocabulary_compression(
                ht;
                cols = [:condition_source_value],
                min_freq = 2,
            )
            ...
            compressed_custom = HealthBase.apply_vocabulary_compression(
                ht;
                cols = [:condition_source_value],
                min_freq = 2,
                other_label = "RARE",
            ...
            @test "RARE" in
                  unique(compressed_custom.source.condition_source_value_compressed)

            # Test with missing column
            @test_throws AssertionError HealthBase.apply_vocabulary_compression(
                ht;
                cols = [:nonexistent_column],
                min_freq = 2,
            )
...
""",
            ...
            ht_mapped = HealthBase.map_concepts(
                ht,
                :gender_concept_id,
                db;
                new_cols = "gender_name",
            )
    ...
    @testset "Edge Cases and Error Handling" begin
            ...
            if !isnothing(ext)
                ...
                if haskey(omop_versions, "v5.4.1")
                    ...
                    if haskey(corrupted_fields, :person_id)
                        ...
                        omop_versions["v5.4.1"] =
                            Dict{Symbol,Any}(:fields => original_fields)
                    end
                    ...
                    if haskey(corrupted_fields_2, :person_id)
                        ...
                        # Temporarily replace the schema
                        corrupted_version_2 =
                            Dict{Symbol,Any}(:fields => corrupted_fields_2)
        ...
        @testset "map_concepts Edge Cases" begin
            ...
            @test_logs (:warn, r"No concept_ids found") HealthBase.map_concepts!(
                ht_empty,
                :empty_col,
                db;
                new_cols = "mapped_empty",
            )

            df_nonexistent = DataFrame(nonexistent_ids = [99999])
            ht_nonexistent =
                HealthBase.HealthTable(df_nonexistent; omop_cdm_version = "v5.4.1")
        ...
        @testset "apply_vocabulary_compression drop_original" begin
            ...
            ht_result = HealthBase.apply_vocabulary_compression(
                ht_compress;
                cols = [:col1, :col2],
                min_freq = 3,
                drop_original = true,
